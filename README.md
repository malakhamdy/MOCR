...
- Recalibrated the front-side normalized field priors from the supplied sample
  geometry only: photo, name, address, and NID regions are now separated more
  cleanly.
- Replaced the old "highest-confidence single OCR box wins" logic with
  field-aware box merging and ranking. This is important for names/addresses
  split across several OCR boxes and for a 14-digit NID split into chunks.
- Added explicit disagreement reporting when the top OCR candidates do not
  agree.
- Added a PaddleOCR adapter that supports both the legacy 2.x `ocr()` API and
  the current 3.x `predict()` result structure (`rec_texts`, `rec_scores`,
  `rec_polys`).
- NID-derived date of birth and gender are exposed as structured derived
  fields; they are derived from the NID structure rather than treated as
  separate OCR fields.
- Updated the PaddleOCR dependency range to the current 3.x line.
- Added unit tests for OCR box merging/ranking.

The supplied real-card images are **not included in this ZIP**. They were used
only as visual/layout references for normalized ROI calibration; the test
fixtures remain synthetic and contain no real identity data.
